...
/* Enunciat:
    Utilitzant un vector de 56 caselles genera les 28 fitxes
    del dominó aleatòriament, tenint en compte que cada dues
...
void comprovarFitxes(int v1[MAXVECTOR],int qttE,int i,int j,bool *trobat,int *pos1,int *pos2) {
    if(v1[0]!=-1) {                                                                                         ///controlo el inicio del vector, si es -1 es que no hay todavía ninguna ficha así que se guarda directamente.
        while(*pos2<qttE && v1[*pos1]!=-1) {                                                                  ///recorremos todo el vector y pararemos si encontramos una marca (-1).
            if((v1[*pos1]==i && v1[*pos2]==j) || (v1[*pos1]==j && v1[*pos2]==i)) (*trobat)=true;                                                                                ///miramos si el primer valor se encuentra.
                else {
                *pos1=*pos1+2; *pos2=*pos2+2;
            }
